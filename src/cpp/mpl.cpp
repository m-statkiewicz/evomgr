...
const std::string Mpl::WHO = "MPL";

Mpl::Mpl (double mi, double lambda, int max_iterations) :
Method(
	WHO,
	"STATIC",
	"mi", mi,
...
Point Mpl::optimize(const std::vector<Point>& initialPoints) {
	...
	while (iter<getIterationsNumber()) {
  	...
  	//Mutate
		...
		//	#pragma acc loop
			...
			tmpPopulation.insert(tmpPopulation.end(), basePopulation.begin(), basePopulation.end());
